...
ns = {'btr':'http://bioagentsregistry.org'}

HOST = 'https://bio.agents'
...
def main():
    ...
    if args.login:
        ...
        resp = requests.delete(HOST + '/api/agent/%s' % args.login, headers={'Accept':'application/json', 'Content-type':'application/json', 'Authorization': 'Token %s' % token}, verify=SSL_VERIFY)
    for filename in filenames:
        ...
        if args.login and args:
            resp = requests.post(HOST + '/api/agent', etree.tostring(xml, pretty_print=True), headers={'Accept':'application/json', 'Content-type':'application/xml', 'Authorization': 'Token %s' % token}, verify=SSL_VERIFY)
